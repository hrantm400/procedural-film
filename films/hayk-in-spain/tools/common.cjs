...
function loadTimeline(file) {
  ...
  // a timeline may declare its own frame size (photo-doodle films are square); mirrored back below
  ...
  const out = normalizeTimeline(tl);
  out.width = tl.width != null && Number(tl.width) > 0 ? Number(tl.width) : 1080;
  out.height = tl.height != null && Number(tl.height) > 0 ? Number(tl.height) : 1920;
  return out;
}
 ...
 */
/**
 * Photo-doodle prelude, loaded after lib.js and before the timeline when present:
 *   src/photos.js  generated by tools/photos.cjs from assets/cut
 *   src/props.js   the shared doodle props
...
async function openPage(browser, files, { scale = 1, prefix = 'page', query = '?render=1', only = null } = {}) {
  ...
  pg.reopen = async () => {
    ...
    pg.info = null;
    await page.evaluate(() => (window.FILM && window.FILM.loadPhotos ? window.FILM.loadPhotos() : 0));
    // The hand-lettering face resolves asynchronously: a frame drawn before it is ready uses a
    // fallback and hashes differently from the same frame drawn later (flaky determinism).
    await page.evaluate(async () => {
      if (!document.fonts) return 0;
      await document.fonts.load('46px "Chalkboard SE", "Marker Felt", "Comic Sans MS", cursive', 'abcdefghijklmnopqrstuvwxyz0123456789!?');
...
// Photographs are the one sanctioned media, and only through the two doors that put them there: the
// generated src/photos.js section, and core's own loader lines marked photo-io. A marker copied into
// a scene file exempts nothing — the scope check below is on the file, not just the comment.
function scanForbidden(html) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const head = lines[i].startsWith('// ---- ') ? lines[i].slice(8).replace(/\\/g, '/') : null;
    if (head !== null) {
      inPhotos = head === 'src/photos.js';
      inCore = head === 'src/core.js';
    } else if (lines[i] === '</script>') {
      inPhotos = false;
      inCore = false;
    }
    if (inPhotos || (inCore && lines[i].includes('/* photo-io */'))) exempt.add(i);
  }
