// STUB
// Placeholder for shot 08 'burger-scent' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'burger-scent',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('burger-scent');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 08', cx, H * 0.17, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'burger-scent', cx, safeBottom - 120, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'burger-scent', cx, safeBottom - 70, { size: 30, align: 'center', color: P.inkSoft });
