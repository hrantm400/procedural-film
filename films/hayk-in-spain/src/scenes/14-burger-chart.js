// STUB
// Placeholder for shot 14 'burger-chart' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'burger-chart',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('burger-chart');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 14', cx, H * 0.17, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'burger-chart', cx, safeBottom - 120, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'burger-chart', cx, safeBottom - 70, { size: 30, align: 'center', color: P.inkSoft });
