// STUB
// Placeholder for shot 03 'big-decision' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'big-decision',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('big-decision');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 03', cx, H * 0.17, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'big-decision', cx, safeBottom - 120, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'big-decision', cx, safeBottom - 70, { size: 30, align: 'center', color: P.inkSoft });
