// STUB
// Placeholder for shot 12 'food-nirvana' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'food-nirvana',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('food-nirvana');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 12', cx, H * 0.17, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'food-nirvana', cx, safeBottom - 120, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'food-nirvana', cx, safeBottom - 70, { size: 30, align: 'center', color: P.inkSoft });
